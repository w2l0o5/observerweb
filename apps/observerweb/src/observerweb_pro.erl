...
get_update(#state{node=Node, sort=Sort=#sort{sort_key=KeyField}} = S0) ->
    ...
    Info = receive
           ...
           after 1000 -> exit(connection_lost) end,
...
accum2(PIs=[#etop_proc_info{pid=Pid}|_], [#etop_proc_info{pid=OldPid}|Old], Acc) when Pid > OldPid ->
    accum2(PIs, Old, Acc);
accum2([PI|PIs], Old, Acc) ->
    accum2(PIs, Old, [PI|Acc]);
accum2([], _, Acc) -> Acc.

sort(Col, Opt, Table) when not is_list(Table) ->
    sort(Col,Opt,array:to_list(Table));
sort(Col, Opt=#sort{sort_key=Col, sort_incr=Bool}, Table) ->
    {Opt#sort{sort_incr=not Bool}, array:from_list(lists:reverse(Table))};
sort(Col, S=#sort{sort_incr=true}, Table) ->
    {S#sort{sort_key=Col}, array:from_list(lists:keysort(col_to_element(Col), Table))};
sort(Col, S=#sort{sort_incr=false}, Table) ->
    {S#sort{sort_key=Col}, array:from_list(lists:reverse(lists:keysort(col_to_element(Col), Table)))}.

get_procinfo_data(Col, Info) -> element(col_to_element(Col), Info).
...
warp_json2(Index, Info, Json) ->
    case Index =:= array:size(Info) of
        ...
        warp_json2(Index+1, Info,
            [{[{<<"pid">>, Pid},{<<"name">>, Name},{<<"reds">>, Reds},{<<"mem">>,Mem},{<<"msg">>,Msg},{<<"fun">>,Fun}]} | Json])
    end.
